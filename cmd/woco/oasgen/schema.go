...
func (sch *Schema) genProperties(c *Config, spec *openapi3.SchemaRef) {
	for _, pname := range sortPropertyKeys(spec.Value.Properties) {
		...
		gs := genSchemaRef(c, sch.SchemaOptions.With(WithSchemaName(pname), WithPrefixName(sch.Name), WithSchemaSpec(schemaRef), WithSchemaRequired(helper.InStrSlice(spec.Value.Required, pname))))
