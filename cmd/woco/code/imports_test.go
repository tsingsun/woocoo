...
func TestImportPathForDir(t *testing.T) {
	...
	assert.Equal(t, "github.com/tsingsun/woocoo/cmd/woco/code", ImportPathForDir(wd))

	assert.Equal(t, "github.com/tsingsun/woocoo/cmd/woco/code", ImportPathForDir(wd))
	assert.Equal(t, "github.com/tsingsun/woocoo/cmd/api", ImportPathForDir(filepath.Join(wd, "..", "..", "api")))

	// doesnt contain go code, but should still give a valid import path
	assert.Equal(t, "github.com/tsingsun/woocoo/cmd/docs", ImportPathForDir(filepath.Join(wd, "..", "..", "docs")))

	// directory does not exist
	assert.Equal(t, "github.com/tsingsun/woocoo/cmd/dos", ImportPathForDir(filepath.Join(wd, "..", "..", "dos")))
...
func TestNameForDir(t *testing.T) {
	...
	assert.Equal(t, "cmd", NameForDir(wd+"/../.."))
}
