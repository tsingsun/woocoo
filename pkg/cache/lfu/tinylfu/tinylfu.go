...
/*
   ...
   This package is a vendored copy of github.com/vmihailenco/go-tinylfu (MIT License, Copyright (c) 2021 Damian Gryski),
   with local modifications to fix same-key eviction and TTL update issues.
...
func (t *T) Set(newItem *Item) {
	if e, ok := t.data[newItem.Key]; ok {
		...
		item.ExpireAt = newItem.ExpireAt
		item.OnEvict = newItem.OnEvict
