...
// Code by prometheus and can see crypto/tls/generate_cert.go
package main
...
func GenerateCertificateAuthority(commonName string, parentCert *x509.Certificate, parentKey *rsa.PrivateKey) (*x509.Certificate, *rsa.PrivateKey, error) {
	...
	caCert := &x509.Certificate{
		...
		Subject: pkix.Name{
			Country:            []string{"CN"},
			Organization:       []string{"WooCoo"},
			OrganizationalUnit: []string{"WooCoo Certificate Authority"},
...
func main() {
	...
	rootCert, rootKey, err := GenerateCertificateAuthority("WooCoo Root CA", nil, nil)
	if err != nil {
		log.Fatal(err)
	}

	log.Println("Generating CA")
	caCert, caKey, err := GenerateCertificateAuthority("WooCoo TLS CA", rootCert, rootKey)
	if err != nil {
		log.Fatal(err)
	}
	...
	if err := writeCertificateAndKey("server", cert, key); err != nil {
		log.Fatal(err)
	}
	...
	if err := writeCertificateAndKey("client", cert, key); err != nil {
		log.Fatal(err)
	}
	...
	if err := writeCertificateAndKey("self-signed-client", cert, key); err != nil {
		log.Fatal(err)
	}
	...
	if err := os.WriteFile("tls-ca-chain.pem", b.Bytes(), 0644); err != nil {
		log.Fatal(err)
	}
}
